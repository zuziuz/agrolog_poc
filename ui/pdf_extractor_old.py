...
class PDFExtractorUI:
    ...
    def _show_verification_summary(self, results: List[ProcessingResult]):
        ...
        for result in results:
            data.append({
                "Type": "Pickup" if result.task_type == TaskType.PICKUP else "Delivery",
                "Task ID": result.task_id,
                "Address": result.address,
                "Using Verified Coordinates": "✅" if result.is_verified else "❌",
                "Original Lat": result.original_lat,
                "Original Lng": result.original_lng,
                "Verified Lat": result.verified_lat if result.is_verified else "N/A",
                "Verified Lng": result.verified_lng if result.is_verified else "N/A"
            })
